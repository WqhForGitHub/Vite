...
export default defineConfig({
  preview: {
    proxy: {
      // ========== 方式一：字符串简写 ==========
      // http://localhost:4173/foo -> http://localhost:4567/foo
      '/foo': 'http://localhost:4567',

      // ========== 方式二：带选项写法 ==========
      // http://localhost:4173/api/bar -> http://jsonplaceholder.typicode.com/bar
      '/api': {
      ...
      // ========== 方式三：正则表达式写法 ==========
      // http://localhost:4173/fallback/ -> http://jsonplaceholder.typicode.com/
      '^/fallback/.*': {
        target: 'http://jsonplaceholder.typicode.com',
        changeOrigin: true,
      ...
      // ========== 方式五：代理 WebSocket ==========
      // ws://localhost:4173/socket.io -> ws://localhost:5174/socket.io
      '/socket.io': {
