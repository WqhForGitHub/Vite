...
更多细节请见 [项目根目录](https://cn.vite.dev/guide/#index-html-and-project-root)。
...
更多信息详见 [公共基础路径](https://cn.vite.dev/guide/build#public-base-path)。
...
查看 [环境变量与模式](https://cn.vite.dev/guide/env-and-mode) 章节获取更多细节。
...
需要用到的插件数组。Falsy 虚值的插件将被忽略，插件数组将被扁平化（flatten）。查看 [插件 API](https://cn.vite.dev/guide/api-plugin) 获取 Vite 插件的更多细节。
...
欲了解更多，请参阅 [`public` 目录](https://cn.vite.dev/guide/assets#the-public-directory)。
...
T定义用于替换 `import` 或 `require` 语句中值的别名。其工作方式类似于 [`@rollup/plugin-alias`](https://github.com/rollup/plugins/tree/master/packages/alias)。

条目的顺序很重要，因为最先定义的规则会首先应用。

当使用文件系统路径的别名时，请始终使用绝对路径。相对路径的别名值会原封不动地被使用，因此无法被正常解析。

更高级的自定义解析方法可以通过 [插件](https://cn.vite.dev/guide/api-plugin) 实现。
>在 SSR 中使用
>如果你已经为 [SSR 外部化的依赖](https://cn.vite.dev/guide/ssr#ssr-externals) 配置了别名，你可能想要为真实的 `node_modules` 包配别名。[Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias) 和 [pnpm](https://pnpm.io/aliases/) 都支持通过 `npm:` 前缀配置别名。
...
启用 tsconfig 路径解析功能。`tsconfig.json` 中的 `paths` 选项将用于解析导入。更多详情请参见[功能](https://cn.vite.dev/guide/features#paths)。

## html.cspNonce

- **类型:** `string`
- **相关:** [Content Security Policy (CSP)](/guide/features#content-security-policy-csp)
...
当使用 [Lightning CSS](https://cn.vite.dev/guide/features#lightning-css) 时，该选项不会产生任何效果。如果要启用该选项，则应该使用 [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) 来替代。
...
搜索是使用 [postcss-load-config](https://github.com/postcss/postcss-load-config) 完成的，只有被支持的文件名才会被加载。默认情况下，不会搜索工作区根目录（或 [项目根目录](https://cn.vite.dev/guide/#index-html-and-project-root)，如果未找到工作区）之外的配置文件。如有必要，您可以指定根目录之外的自定义路径来加载特定的配置文件。
...
>导入文件
>同一代码段被添加到不同目录的文件头部时，相对路径解析会出错。建议使用绝对路径或[aliases](https://cn.vite.dev/config/shared-options#resolve-alias)。
...
- **实验性:** [Give Feedback](https://github.com/vitejs/vite/discussions/13845)
- **类型:** `boolean`
- **默认:** `false`
...
- **实验性:** [Give Feedback](https://github.com/vitejs/vite/discussions/13835)
- **类型:** `'postcss' | 'lightningcss'`
- **默认:** `'postcss'`

该选项用于选择用于 CSS 处理的引擎。详细信息请查看 [Lightning CSS](https://cn.vite.dev/guide/features#lightning-css)。
...
- **相关内容:** [Static Asset Handling](/guide/assets)
...
关于环境文件的更多信息，请参见 [这里](https://cn.vite.dev/guide/env-and-mode#env-files)。
...
>如果你想暴露一个不含前缀的变量，可以使用 [define](https://cn.vite.dev/config/shared-options#define) 选项：
>```javascript
>define: {
...
无论你的应用是一个单页应用（SPA）还是一个 [多页应用（MPA）](https://cn.vite.dev/guide/build#multi-page-app)，亦或是一个定制化应用（SSR 和自定义 HTML 处理的框架）：

- `'spa'`：包含 HTML 中间件以及使用 SPA 回退。在预览中将 [sirv](https://github.com/lukeed/sirv) 配置为 `single: true`
- `'mpa'`：包含 HTML 中间件
- `'custom'`：不包含 HTML 中间件

要了解更多，请查看 Vite 的 [SSR 指引](https://cn.vite.dev/guide/ssr#vite-cli)。相关内容：[`server.middlewareMode`](https://cn.vite.dev/config/server-options#server-middlewaremode)。

## devtools

- **实验性:** [Give Feedback](https://github.com/vitejs/devtools/discussions)
...
- **默认:** [Breaking Changes](/changes/)

启用未来的重大变更，为顺利迁移到 Vite 的下一个主要版本做好准备。随着新功能的开发，这个列表可能会随时进行更新、添加或移除。

请查看 [破坏性变更](https://cn.vite.dev/changes/) 页面，了解可能的选项详情。