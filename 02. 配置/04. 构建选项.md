...
- **相关内容:**  [浏览器兼容性](https://cn.vite.dev/guide/build#browser-compatibility)
...
注意：此 polyfill **不适用于** [Library 模式](https://cn.vite.dev/guide/build#library-mode)。如果你需要支持不支持动态引入的浏览器，你应该避免在你的库中使用此选项。
...
解析得到的依赖路径可以再在之后使用 [`experimental.renderBuiltUrl`](https://cn.vite.dev/guide/build#advanced-base-options) 更改。

## build.polyfillModulePreload

- **类型:** `boolean`
- **默认:** `true`
- **已废弃** use `build.modulePreload.polyfill` instead
...
指定输出路径（相对于 [项目根目录](https://cn.vite.dev/guide/#index-html-and-project-root)).
...
指定生成静态资源的存放路径（相对于 `build.outDir`）。在 [库模式](https://cn.vite.dev/guide/build#library-mode) 下不能使用。
...
- **类型:** `string | string[]`
- **默认值:** 与 [`build.target`](https://cn.vite.dev/config/build-options#build-target) 一致
...
- **默认:**  `'lightningcss'`，但如果客户端构建时禁用了 [`build.minify`](https://cn.vite.dev/config/build-options#build-minify)，则为 `false`

此选项允许用户覆盖 CSS 最小化压缩的配置，而不是使用默认的 `build.minify`，这样你就可以单独配置 JS 和 CSS 的最小压缩方式。Vite 默认使用 [Lightning CSS](https://lightningcss.dev/minification.html) 来压缩 CSS。可以通过 [`css.lightningcss`](https://cn.vite.dev/config/shared-options#css-lightningcss) 进行配置。将此选项设置为 `'esbuild'` 可以改用 esbuild 进行压缩。
...
如果您正在编写插件，并且需要在构建过程中检查每个输出块或资源的 CSS 和静态资源，您也可以使用 [`viteMetadata` 构建输出元数据 API](https://cn.vite.dev/guide/api-plugin#output-bundle-metadata).
...
- **默认:** `'oxc'` for client build, `false` for SSR build

置为 `false` 可以禁用最小化混淆，或是用来指定使用哪种混淆器。默认使用 [Oxc Minifier](https://oxc.rs/docs/guide/usage/minifier)，它比 terser 快 30~90 倍，但压缩率仅差 0.5~2%。[基准测试](https://github.com/privatenumber/minification-benchmarks)
...
设置为 `false` 来禁用将构建后的文件写入磁盘。这常用于 [编程式地调用 `build()`](https://cn.vite.dev/guide/api-javascript#build) 在写入磁盘之前，需要对构建后的文件进行进一步处理。
...
>某些情况下 WSL2 的文件系统监听可能无法正常工作。 查看 [`server.watch`](https://cn.vite.dev/config/server-options#server-watch) 了解更多细节。