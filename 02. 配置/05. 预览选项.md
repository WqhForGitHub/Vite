...
>注意
>在某些情况下，可能响应的是其他服务器而不是 Vite。 查看 [`server.host`](https://cn.vite.dev/config/server-options#server-host) 了解更多细节。
...
查看 [`server.allowedHosts`](https://cn.vite.dev/config/server-options#server-allowedhosts) 以获取更多详细信息。
...
有关更多详细信息，请参阅 [`server.https`](https://cn.vite.dev/config/server-options#server-https)。
...
请查看 [`server.cors`](https://cn.vite.dev/config/server-options#server-cors) 了解更多细节。
