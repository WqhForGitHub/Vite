- 相关: [公共基础路径](https://cn.vite.dev/guide/build#public-base-path)
- 相关: [`assetsInclude` 配置项](https://cn.vite.dev/config/shared-options#assetsinclude)
# 将资源引入为 URL
...
- 常见的图像、媒体和字体文件类型被自动检测为资源。你可以使用 [`assetsInclude` 选项](https://cn.vite.dev/config/shared-options#assetsinclude) 扩展内部列表。
    
- 引用的资源作为构建资源图的一部分包括在内，将生成散列文件名，并可以由插件进行处理以进行优化。
    
- 较小的资源体积小于 [`assetsInlineLimit` 选项值](https://cn.vite.dev/config/build-options#build-assetsinlinelimit) 则会被内联为 base64 data URL。
    
- Git LFS 占位符会自动排除在内联之外，因为它们不包含它们所表示的文件的内容。要获得内联，请确保在构建之前通过 Git LFS 下载文件内容。
    
- 默认情况下，TypeScript 不会将静态资源导入视为有效的模块。要解决这个问题，需要添加 [`vite/client`](https://cn.vite.dev/guide/features#client-types)。
>通过 `url()` 内联 SVG
...
```javascript
// sharedworker
import ShareWorker from './shader.js?sharedworker'
...
```
查看 [Web Worker 小节](https://cn.vite.dev/guide/features#web-workers) 获取更多细节。
# public 目录
...
目录默认是 `<root>/public`，但可以通过 [`publicDir` 选项](https://cn.vite.dev/config/shared-options#publicdir) 来配置。

请注意，应该始终使用根绝对路径来引入 `public` 中的资源 —— 举个例子，`public/icon.png` 应该在源码中被引用为 `/icon.png`。
>在导入目录和 `public` 目录之间进行选择
...
>```javascript
>import __imgOpng from './dir/img0.png'
