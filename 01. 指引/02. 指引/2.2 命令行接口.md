...
| `--profile`               | 启动内置的 Node.js 调试器（查看 [性能瓶颈](https://cn.vite.dev/guide/troubleshooting#performance-bottlenecks)） |
...
此命令在构建目录（默认为 `dist`）中启动服务器。请提前运行 `vite build` 以确保构建目录是最新的。根据项目配置的 [`appType`](https://cn.vite.dev/config/shared-options#apptype)，它会使用某些中间件。
